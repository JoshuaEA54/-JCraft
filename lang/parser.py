...
class Parser:
    ...
    def expect(self, ttype: str, value: Optional[str] = None) -> Token:
        ...
        if tok.type != ttype:
            raise ParserError(f"Expected {ttype} but got {tok.type} at {tok.line}:{tok.column}")
        if value is not None and tok.value != value:
            raise ParserError(f"Expected {value} but got {tok.value} at {tok.line}:{tok.column}")
        return self.advance()
    ...
    def parse_declaration(self) -> Optional[ASTNode]:
        ...
        if tok.type == 'KEYWORD' and tok.value in ('bloques', 'coordenada', 'texto', 'redstone', 'glifo', 'inventario', 'mapa'):
            ...
            if tok2 and tok2.type == 'OP' and tok2.value == '=':
                ...
                raise ParserError(f"Expected '=' after variable declaration at {tok.line}:{tok.column}")
    ...
    def _expect_type_token(self) -> str:
        ...
        if tok.type == 'KEYWORD':
            self.advance()
            return tok.value
        raise ParserError(f"Expected type keyword but got {tok.type} at {tok.line}:{tok.column}")
    ...
    def parse_param_list(self) -> List[tuple]:
        ...
        while True:
            ...
            name_tok = self.expect('IDENT')
            params.append((typ, name_tok.value))
    ...
    def parse_statement(self) -> Optional[ASTNode]:
        ...
        raise ParserError(f"Unexpected statement starting with {tok.type}({tok.value}) at {tok.line}:{tok.column}")
    ...
    def parse_logic(self):
        ...
        while self.current() and self.current().type == 'KEYWORD' and self.current().value in ('y', 'o'):
